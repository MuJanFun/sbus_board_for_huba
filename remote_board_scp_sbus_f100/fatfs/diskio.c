...
)
{
    ...
    if(drv)
    {
        if(0==ch375_initdisk()) return 0;
		  else return 1;
    }

    state = ch375_initdisk();
    if(state == STA_NODISK)
    {
        return STA_NODISK;
    }
...
)
{
    ...
    if (drv)
    {    
        res = ch375ReadSector(sector,buff,count);
	    if(res == 0x00)
	    {
	        return RES_OK;
	    }
    ...
    {                                                
		...
		res=ch375ReadSector(sector,buff,1);      
    }                                                
    else                    //多个sector的读操作     
    {                                                
...
)
{
    ...
    if (drv)
    {    
        res = ch375WriteSector(sector,buff,count);
		if(res == 0)
	    {
	        return RES_OK;
	    }
    ...
    if(count == 1)
    {
        res = ch375WriteSector(sector,buff,1);
    }
    else
    {
        res = ch375WriteSector(sector,buff,count);
...
)
{
    ...
    if (drv)
    {    
        return RES_OK;//return RES_PARERR;  //仅支持单磁盘操作，否则返回参数错误
    }
