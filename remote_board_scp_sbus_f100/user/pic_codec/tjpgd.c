...
)
{
	...
	for (blk = 0; blk < nby + nbc; blk++) {
		...
		if (b < 0) return JDR_INP;//if (b < 0) return 0 - b;				/* Err: invalid code or input */
		d = jd->dcv[cmp];						/* DC value of previous block */
		if (b) {								/* If there is any difference from previous block */
			e = bitext(jd, b);					/* Extract data bits */
			if (e < 0) return JDR_INP;//if (e < 0) return 0 - e;			/* Err: input */
		...
		do {
			...
			if (b == 0) break;					/* EOB? */
			if (b < 0) return JDR_INP;			/* Err: invalid code or input error */
			...
			if (b &= 0x0F) {					/* Bit length */
				...
				if (d < 0) return JDR_INP;		/* Err: input device */
				b = 1 << (b - 1);				/* MSB position */
